...
axios
  .get(
    "http://localhost:9090/getFeedbackByMonth" 
    )
  .then(response => {
 ...
 labels: lab,
  ...
  .catch((err) => {
   ...
   axios.get("http://localhost:9090/getFeedbackDetailsForEvent")
   .then(response=>{
     console.log(response.data)
     this.feedbackdetails= response.data;
     this.feedbackdetails.map((feedback,index)=>{
     
       if(index==0){
        
        this.setState({feedCount:feedback})
        
       }else{
this.setState({totalFeedbackExp:feedback})
       }
     })
                  ...
                  <tbody>
                    {
                      ...
                      this.state.totalFeedbackExp.map((feed,index)=>{
                      
                      if(index<= this.state.feedCount.length){
                        console.log(this.state.feedCount)
                        if(this.state.feedCount[index][1]==feed[1]){
                          this.eventid=feed[1];
                          this.total=feed[0];
                          this.actual=this.state.feedCount[index][0]
                          this.poc=feed[2]
